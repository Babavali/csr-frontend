...
import LeadListingComponent from './components/leadListingComponent'
import noData from '../../../assets/no-data.svg'
import EventIcon from '@mui/icons-material/Event'
import _ from 'lodash'
import TransferMultipleLeadsModalPopup from './components/transferMultipleLeadsModalPopup'
import {
	...
	// setLeadListingPatientCountryName,
...
} from '../../../slices/leadFilterSlice'
import { useDispatch } from 'react-redux'
import PartnerComponent from './components/partnerComponent.jsx'
...
function MyLeads() {
	...
	// Redux
	const filterObj = useSelector((state) => state.filterReducer)
	const filterChips = filterObj.leadListingFilter
		...
		let replacedKeys = Object.keys(queryObjectToBeMapped)
			...
			.map((key) => {
				...
				if (newKey === 'lead_date_from') {
					...
					dispatch(setLeadListingLeadGeneratedDateFrom(tempObj))
				}
				// Lead Date To
				if (newKey === 'lead_date_to') {
					...
					dispatch(setLeadListingLeadGeneratedDateTo(tempObj))
				}
				...
				if (newKey === 'status') {
					...
					dispatch(setLeadListingMainStatus(tempObj))
				}
				// Sub Status
				if (newKey === 'sub_status') {
					...
					dispatch(setLeadListingSubStatus(tempObj))
				}
				...
				if (newKey === 'enquirer_name') {
					...
					dispatch(setLeadListingEnquirerName(tempObj))
				}
				// Enquirer Phone
				if (newKey === 'enquirer_phone') {
					...
					dispatch(setLeadListingEnquirerPhone(tempObj))
				}
				...
				if (newKey === 'patient_name') {
					...
					dispatch(setLeadListingPatientName(tempObj))
				}
				// Patient Gender
				if (newKey === 'patient_gender') {
					...
					dispatch(setLeadListingPatientGender(tempObj))
				}
				// Patient DOB
				if (newKey === 'patient_birth_date') {
					...
					dispatch(setLeadListingPatientDob(tempObj))
				}
				...
				if (newKey === 'patient_country_id') {
					...
					dispatch(setLeadListingPatientCountry(tempObj))
				}
				...
				if (newKey === 'only_international_leads') {
					...
					dispatch(setLeadListingPatientIsInternationalOnly(tempObj))
				}
				...
				if (newKey === 'ppc_hospital_name') {
					...
					dispatch(setLeadListingPPCWebsite(tempObj))
				}
				// Category
				if (newKey === 'category_id') {
					...
					dispatch(setLeadListingCategory(tempObj))
				}
				// Specialization
				if (newKey === 'specialization_id') {
					...
					dispatch(setLeadListingSpecialization(tempObj))
				}
				// Service
				if (newKey === 'service_id') {
					...
					dispatch(setLeadListingService(tempObj))
				}
				// Lead ID
				if (newKey === 'lead_id') {
					const tempObj = {
					...
					dispatch(setLeadListingLeadId(tempObj))
				}
				...
				if (newKey === 'lead_tier') {
					...
					dispatch(setLeadListingTier(tempObj))
				}
				...
				if (newKey === 'lead_source') {
					...
					dispatch(setLeadListingLeadSource(tempObj))
				}
				// Treatment Intensity
				if (newKey === 'treatment_intensity') {
					...
					dispatch(setLeadListingTreatmentIntensity(tempObj))
				}
	...
	const handleClearFilter = () => {
		...
		setQueryParams(queryParams)
		dispatch(resetLeadListingFilter())
		// fetchLeadListingData()
	}
	...
	const handleDeleteFilterChip = (filterKey, filterObj) => {
		...
		if (filterKey === 'patientCountry') {
			// dispatch(setLeadListingPatientCountryName(tempObj))
			dispatch(setLeadListingPatientCountry(tempObj))
		}
		// Patient Is International Only
	...
	const handleLeadListingTabChange = (_event, newTabIndex) => {
		...
		if (newTabIndex === 0) {
			navigate(`/leads?type=all&page=${allLeadPage}`)
		} else if (newTabIndex === 1) {
			navigate(`/leads?type=fresh&page=${freshLeadPage}`)
		} else if (newTabIndex === 2) {
			navigate(`/leads?type=working&page=${workingLeadPage}`)
		} else if (newTabIndex === 3) {
			navigate(`/leads?type=stale&page=${staleLeadPage}`)
		}
	}
	...
	const fetchLeadListingData = useCallback(() => {
		...
		let url
		const salesLeadURLPath = 'sales/leads/sale-emp'
