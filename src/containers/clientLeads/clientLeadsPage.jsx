...
export default function clientLeadsPage() {
	...
	const handleLeadListingTabChange = (_event, newTabIndex) => {
		...
		if (newTabIndex === 0) {
			navigate(`/client-leads?type=all&page=${allLeadPage}`)
		} else if (newTabIndex === 1) {
			navigate(`/client-leads?type=fresh&page=${freshLeadPage}`)
		} else if (newTabIndex === 2) {
			navigate(`/client-leads?type=working&page=${workingLeadPage}`)
		} else if (newTabIndex === 3) {
			navigate(`/client-leads?type=stale&page=${staleLeadPage}`)
		}
	}
	...
	const fetchLeadListingData = useCallback(() => {
		...
		if (!_.isEmpty(trialFilterParams)) {
			...
			if (filterChips.leadSource.value === null) {
				queryParams.set('lead_source', 'CLIENT_SEO,CLIENT_SEO_MANUAL,CLIENT_PPC,CLIENT_PPC_MANUAL,CLIENT_FB,CLIENT_GMB')
			}
			setQueryParams(queryParams)
		}
	...
	}, [queryParams, leadListingTabIndex, allLeadPage, freshLeadPage, selectedSalesEmpId, selectedPartnerId, user.isAdmin, user.userId])

	useEffect(() => {
		if (leadListingTabIndex === 0) {
			navigate(`/client-leads/leads?${queryParams}`)
		}
		fetchLeadListingData()
