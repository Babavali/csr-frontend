...
import FacebookLeadsFilterComponent from './components/facebookLeadsFilterComponent'
...
export default function FacebookLeadsPage() {
	...
	const [domainLevel, setDomainLevel] = useState(null)
	...
	const handleLeadListingTabChange = (_event, newTabIndex) => {
		...
		if (newTabIndex === 0) {
			navigate(`/offline/leads?type=all&page=${allLeadPage}`)
		} else if (newTabIndex === 1) {
			navigate(`/offline/leads?type=fresh&page=${freshLeadPage}`)
		} else if (newTabIndex === 2) {
			navigate(`/offline/leads?type=working&page=${workingLeadPage}`)
		} else if (newTabIndex === 3) {
			navigate(`/offline/leads?type=stale&page=${staleLeadPage}`)
		}
	}
	...
	const fetchLeadListingData = useCallback(() => {
		...
		if (!_.isEmpty(trialFilterParams)) {
			...
			if (domainLevel === 'INTERNATIONAL' && filterChips.leadSource.value === null) {
				queryParams.set('lead_source', 'PARTNER_INT_FB,PARTNER_MANUAL')
			}
			if (domainLevel === 'DOMESTIC' && filterChips.leadSource.value === null) {
				queryParams.set(
					'lead_source',
					'PARTNER_SEO,PARTNER_SEO_MANUAL,PARTNER_PPC,PARTNER_PPC_MANUAL,PARTNER_GMB,PARTNER_CALL,PARTNER_DOM_FB'
				)
			}
			if (domainLevel === null && filterChips.leadSource.value === null) {
				queryParams.set(
					'lead_source',
					'FACEBOOK,PARTNER,OPD_CAMP,PARTNER_INT_FB,PARTNER_MANUAL,PARTNER_SEO,PARTNER_SEO_MANUAL,PARTNER_PPC,PARTNER_PPC_MANUAL,PARTNER_GMB,PARTNER_CALL,PARTNER_DOM_FB'
				)
			}
			setQueryParams(queryParams)
		}
		...
		axios
			...
			.then((res) => {
				...
				if (res.data.data.segregation === 'PARTNER_INT_FB,PARTNER_MANUAL') {
					setDomainCheck('INTERNATIONAL')
				} else if (
					res.data.data.segregation ===
					'PARTNER_SEO,PARTNER_SEO_MANUAL,PARTNER_PPC,PARTNER_PPC_MANUAL,PARTNER_GMB,PARTNER_CALL,PARTNER_DOM_FB'
				) {
					setDomainCheck('DOMESTIC')
				} else {
					setDomainCheck(null)
				}
			})
	...
	}, [queryParams, leadListingTabIndex, allLeadPage, freshLeadPage, selectedSalesEmpId, user.isAdmin, user.userId, domainLevel])

	useEffect(() => {
		if (leadListingTabIndex === 0) {
			navigate(`/offline/leads?${queryParams}`)
		}
		fetchLeadListingData()
	}, [
		queryParams,
	...
	useEffect(() => {
		if (leadListingResponse != null) {
			if (leadListingTabArr[leadListingTabIndex] === leadListingResponse.type && domainLevel === domainCheck) {
				...
				<Box
					...
					<Box
						...
						<FormControl sx={{ width: '150px' }}>
							<InputLabel id="demo-simple-select-label">Domain Level</InputLabel>
							<Select
								labelId="demo-simple-select-label"
								id="demo-simple-select"
								value={domainLevel === null ? 'ALL' : domainLevel}
								label="Domain Level"
								onChange={(event) => {
									event.target.value === 'ALL' ? setDomainLevel(null) : setDomainLevel(event.target.value)
								}}>
